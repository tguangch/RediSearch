...
typedef struct Fragment {
  ...
  float score;
  Buffer termLocs;  // Buffer<TermLoc>
} Fragment;
...
static inline void FragmentList_Init(FragmentList *fragList, const FragmentTerm *terms,
                                     uint32_t numTerms, Stemmer *stemmer, uint16_t maxDistance,
                                     uint8_t estWordSize) {
  fragList->doc = NULL;
  fragList->docLen = 0;
  ...
  fragList->stemmer = stemmer;
  Buffer_Init(&fragList->frags, sizeof(Fragment) * 8);
}

static inline size_t FragmentList_GetNumFrags(const FragmentList *fragList) {
  return BUFFER_GETSIZE_AS(&fragList->frags, Fragment);
}

static const Fragment *FragmentList_GetFragments(const FragmentList *fragList) {
  return (Fragment *)Buffer_GetData(&fragList->frags);
}
